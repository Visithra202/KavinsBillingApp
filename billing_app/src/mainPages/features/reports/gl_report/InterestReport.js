...
export default function InterestReport() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/get-report-interest/')
      .then((response) => {
        setReportList(response.data)
        setLoading(false);
      }).catch((error) => {
        // console.error('Error fetching Interest report ' + error.response.data)
        setLoading(false);
      })
  }, [])
      ...
      <div className='border border-secondary bg-white rounded-2 shadow  my-2 scroll-bar'
        ...
        <table className='cashreport itmlst table table-hover'>
          <thead className=' rounded-top-5' style={{ position: 'sticky', top: '0', zIndex: '1', }}>
            <tr>
              <th>Date</th>
              <th>Transaction comment - Interest Income report </th>
          ...
          <tbody className='px-4 py-1'>
            {loading ?
              ...
              : reportList.length > 0 ? (
                ...
                <tr className='text-center'><td colSpan='9'>Interest report not Found</td></tr>
              )}
          </tbody>
        </table>
      </div>
